/*
 * shader-background.js — vanilla port of the provided React ShaderBackground.
 * Target: plain HTML/JS QR Menu (no React/bundler). All optimizations preserved:
 *   - DPR capped at 2
 *   - pixel budget ~2,000,000 (scales down render resolution)
 *   - render only while visible (document.visibilityState) and in view (IntersectionObserver)
 *   - ResizeObserver for responsive canvas
 *   - WebGL context cleanup via WEBGL_lose_context
 * Same UNIFORMS / colors as the original:
 *   colors: #0B1017-ish deep navy, indigo, lilac, soft pink (4 colours used)
 */
(function () {
  'use strict';
...
void main() {
  ...
  const UNIFORMS = {
    colors: [
      ...
      [0.6941176470588235, 0.5490196078431373, 1.0],
      [1.0, 0.8392156862745098, 0.9058823529411765],
      [1.0, 0.8392156862745098, 0.9058823529411765],
      [1.0, 0.8392156862745098, 0.9058823529411765],
      [1.0, 0.8392156862745098, 0.9058823529411765],
      [1.0, 0.8392156862745098, 0.9058823529411765],
  ...
  function initShader(canvas) {
    ...
    const updatePointerTarget = () => {
      ...
      targetX = nextX; targetY = nextY; targetPresence = 1; requestRender();
    };
    ...
    function render(now) {
      ...
      if (timeAnimated || settling) requestRender();
      else lastNow = null;
    }
    resizeCanvas();
    requestRender();
  ...
  // Init on DOM ready for every canvas.shader-background
  function boot() {
    document.querySelectorAll('canvas.shader-background').forEach((c) => {
      if (!c.__shaderInited) { c.__shaderInited = true; initShader(c); }
    });
  }
